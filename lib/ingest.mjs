...
export async function ingestMap(mapUrl, opts = {}) {
  ...
  // Locations + regions used to be embedded in window.mapData, but the source
  // now serves them from a per-map data endpoint while the page carries only
  // the group/category tree and map config. Pull them from that endpoint
  // whenever they aren't embedded (the current norm); groups stay sourced from
  // the page, since the data endpoint doesn't include them.
  let rawLocations = mapData.locations;
  let rawRegions = mapData.regions;
  if (!Array.isArray(rawLocations)) {
    const mapId = mapData.map?.id;
    if (!mapId) throw new Error("could not find the map id to fetch locations");
    emit("page", 0, 1, "fetching locations");
    const dataUrl = `${new URL(PAGE_URL).origin}/api/v1/maps/${mapId}/data`;
    const dataRes = await fetchWithRetry(dataUrl, {
      headers: { Accept: "application/json", "X-Requested-With": "XMLHttpRequest" },
    });
    if (!dataRes || !dataRes.ok) {
      throw new Error(`map data fetch failed: HTTP ${dataRes?.status ?? "?"}`);
    }
    const apiData = await dataRes.json();
    rawLocations = apiData.locations || [];
    if (!Array.isArray(rawRegions)) rawRegions = apiData.regions || [];
    emit("page", 1, 1, `fetched ${rawLocations.length} locations`);
  }
  ...
  // The source's tile axis order varies by map — the declared pattern is the
  // source of truth (older maps serve {z}/{y}/{x}, newer ones {z}/{x}/{y}).
  // Build the fetch URL from it. Tiles are always STORED below in the app's
  // canonical {z}/{y}/{x} layout regardless of the source order.
  const tilePattern = tileSet.pattern || `${tileSet.path}/{z}/{y}/{x}.${ext}`;
  const tileUrl = (z, x, y) =>
    tilesCdnUrl +
    tilePattern.replace("{z}", z).replace("{x}", x).replace("{y}", y);
  ...
  for (let z = minZoom; z <= maxZoom; z++) {
    ...
    await pool(
      ...
      async ({ z, x, y }) => {
        ...
        const r = await fetchWithRetry(tileUrl(z, x, y));
        if (r && r.ok) {
